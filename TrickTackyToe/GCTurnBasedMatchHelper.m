...
- (void)authenticateLocalUser {
	...
	NSLog(@"I: %@",[[GKLocalPlayer localPlayer] displayName]);
...
//	NSLog(@"fsize %d", friends.count);

//	for (GKPlayer *friend in friends) {
//		NSLog(@"Friend: %@", friend.alias);
//	}
}

// Gather all your friends
-(void) loadPlayerData: (NSArray *) identifiers {
	NSMutableArray *myFriends = [[NSMutableArray alloc] initWithCapacity:1];
	[GKPlayer loadPlayersForIdentifiers:identifiers withCompletionHandler:^(NSArray *players, NSError *error) {
		 ...
		 if (players != nil) {
			 ...
			 for (GKPlayer *friend in players) {
				NSLog(@"I have a friend named %@", [friend alias]);
